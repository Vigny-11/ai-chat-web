...
export class OpenAIProvider implements ServerAIProvider {
  id = 'openai' as const
  name = 'OpenAI'
  private baseUrl = process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1/chat/completions'
  ...
  chat(apiKey: string, messages: unknown[], stream: boolean, maxTokens = defaultOptions.maxTokens) {
    return withTimeout(this.baseUrl, {
      method: 'POST',
