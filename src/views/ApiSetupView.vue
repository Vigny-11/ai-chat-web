...
const startEnabled = computed(() => !loading.value && Boolean(apiKey.value.trim()))

const resetConnection = () => {
  app.markApiConnected(false)
  result.value = ''
  failed.value = false
  console.log('enabled', startEnabled.value)
}
    ...
    <section class="panel rounded-md p-6 shadow-soft">
      ...
      <div class="mt-6 grid gap-4">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          ...
          <button class="btn-primary" :disabled="!startEnabled" @click="start">开始使用</button>
        </div>
      </div>
    </section>
  </main>
