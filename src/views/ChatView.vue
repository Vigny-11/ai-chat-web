...
const displayContent = (message: { content?: unknown; text?: unknown; message?: unknown; isGenerating?: boolean }) => {
  return extractMessageText(message) || (message.isGenerating ? '正在输入...' : '')
}
    ...
    <aside :class="['panel fixed inset-y-0 left-0 z-40 w-72 translate-x-[-100%] overflow-y-auto p-4 transition lg:static lg:translate-x-0', leftOpen && 'translate-x-0']">
      ...
      <div v-if="character" class="mt-5">
        ...
        <div class="mt-3 grid gap-2">
          <button v-for="conversation in chat.conversations" :key="conversation.id" class="rounded-md p-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-900" @click="chat.loadMessages(conversation.id)">
          ...
          <article v-for="msg in chat.messages" :key="msg.id" :class="['flex gap-3', msg.role === 'user' ? 'justify-end' : 'justify-start']">
            ...
            <div :class="['max-w-[86%] rounded-md px-4 py-3 text-sm shadow-sm', msg.role === 'user' ? 'bg-moss text-white' : 'bg-white text-slate-800 dark:bg-slate-900 dark:text-slate-100']">
              ...
              <div class="mt-2 flex flex-wrap items-center gap-2 text-xs opacity-70">
                ...
                <button v-if="msg.role === 'assistant'" @click="copyText(displayContent(msg))"><Copy class="h-3.5 w-3.5" /></button>
                <button v-if="msg.role === 'user'" @click="input = displayContent(msg)"><Pencil class="h-3.5 w-3.5" /></button>
